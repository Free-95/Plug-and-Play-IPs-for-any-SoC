...
## 1. RTL Instantiation
Add `gpio_control_ip.v` to your project source list. Instantiate the module in your top-level SoC wrapper (`riscv.v`) as follows:
...
* **Recommended Logic:** Map to `0x400000`.
* **Verilog Snippet:**
        ```verilog
        wire isIO = mem_addr[22]; 
        wire gpio_select = isIO & (mem_addr[7:4] == 4'h0); // Base: 0x400000
        ```
...
* **Example Mapping:**
        ```text
        set_io gpio_pins[0] 28  # LED 0
        set_io gpio_pins[1] 38  # LED 1
        set_io gpio_pins[2] 42  # Button Input
        set_io gpio_pins[3] 36  # Header Pin
        ```
